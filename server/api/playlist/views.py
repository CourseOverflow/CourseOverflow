...
def getPlaylist(request):
    ...
    playlist_data['authorName'] = author_data['first_name'] + " " + \
...
def search(request):
    ...
    for playlist in serializer.data:
        ...
        playlist['authorName'] = author_data['first_name'] + " " + \
            author_data['last_name']
    ...
    if request.user.is_authenticated:
        user = request.user
        for playlist in serializer.data:
            playlist_instance = Playlist.objects.get(
                id=playlist['id'])
            playlist_interaction, _ = PlaylistInteraction.objects.get_or_create(
                userId=user,
                playlistId=playlist_instance
            )
...
def user_playlists(request):
    ...
    for playlist_data in serializer.data:
        ...
        playlist_data['authorName'] = author.first_name + \
            " " + author.last_name
...
def watch_count(request):
    ...
    return Response({'watchCount': watchCount})
...
def create_playlist(request):
    ...
    try:
        ...
        if draft.authorId.id != user.id:
            return Response({'message': 'User unauthorized to create this playlist'}, status=401)
